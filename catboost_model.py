...
# -----------------------------
importances = cb_base.get_feature_importance()
...
for k in k_values:
    ...
    cb.fit(X_train[top_features], y_train)

    y_train_pred = cb.predict(X_train[top_features])
    y_val_pred = cb.predict(X_val[top_features])
...
# -----------------------------
print("\n" + "="*80)
print("R² SCORES BY NUMBER OF FEATURES (CATBOOST)")
print("="*80)
...
print("\n" + "="*80)
print("BEST PERFORMANCE (CATBOOST)")
print("="*80)
...
for i, feat in enumerate(top_features_best[:20], 1):
    importance = feat_importance_df.loc[
        feat_importance_df["feature"] == feat, "importance"
    ].values[0]
...
plt.ylabel("R² Score")
plt.title("CatBoost: Train vs Validation R² by Feature Count")
plt.legend()
plt.grid(True)
plt.tight_layout()
plt.show()
